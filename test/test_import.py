...
Tests that:
1. Given .aictx sources, deploy creates native tool files
2. Import reads those native files back into .aictx
3. Re-deploying from the imported .aictx produces equivalent native files
...
class TestRoundtrip:
    def test_deploy_import_redeploy_equivalence(self):
        """Full roundtrip: deploy from .aictx → import → redeploy → compare."""
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            # Copy .aictx source files from fixture
            ...
            # Phase 2: Delete .aictx files
            for aictx_path in root.rglob(".context.aictx"):
                aictx_path.unlink()

            # Phase 3: Import (reconstructs .aictx from native files)
            imports = [
                r for r in [
            ...
            # Verify .aictx files were created
            imported_aictx = list(root.rglob(".context.aictx"))
            assert len(imported_aictx) >= 1, "Import should create at least root .aictx"

            # Phase 4: Delete native files, redeploy from imported .aictx
...
class TestHandWritten:
    ...
    def test_roundtrip_hand_written_to_aictx_to_native(self):
        """Hand-written CLAUDE.md → import → .aictx → deploy → compare."""
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            ...
            # Delete original, deploy from .aictx
...
class TestCLI:
    def test_import_cli_dry_run(self):
        ...
        assert "Generated 5 .context.aictx" in result.output
