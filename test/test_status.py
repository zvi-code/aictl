...
class TestDiscoverAictl:
    def test_finds_aictx(self, tmp_path):
        ...
        assert aictl
        assert any(".context.aictx" in f.path for f in aictl[0].files)
...
class TestDiscoverAzureAI:
    def test_finds_azure_yaml(self, tmp_path):
        ...
        res = _discover_tool(tmp_path, "azure-ai")
        assert any("config" in f.kind or "manifest" in f.kind for f in res.files)

    def test_empty_dir(self, tmp_path):
        res = _discover_tool(tmp_path, "azure-ai")
        assert res.tool == "azure-ai"
