...
def emit(root: Path, resolved: Resolved, dry_run: bool = False) -> list[dict]:
    ...
    for scope in resolved.scopes:
        ...
        if scope.is_root:
            # Root base → GEMINI.md
            if scope.base:
                emit_file(root / "GEMINI.md", wrap_deployed(scope.base, src), dry_run, results)

            # Root profile → GEMINI.local.md (with overlay)
            if scope.profile_text and resolved.profile:
                fp = root / "GEMINI.local.md"
            ...
            # Sub-scope → .gemini/rules/{scope}.md (base + profile merged)
            if combined:
                safe = encode_scope(src).replace("--", "-")
                glob = f"{src}/**"
                emit_file(root / ".gemini" / "rules" / f"{safe}.md",
    ...
    for cap in resolved.capabilities:
        if cap.kind == "command":
            emit_file(root / ".gemini" / "commands" / f"{cap.name}.md", cap.content, dry_run, results)
        elif cap.kind == "skill":
            emit_file(root / ".gemini" / "skills" / cap.name / "SKILL.md", cap.content, dry_run, results)
    ...
    # --- Ignores → .geminiignore ---
    if resolved.ignores:
        fp = root / ".geminiignore"
