"""Scan a directory tree for .context.aictx files starting from root."""
...
def scan(root: Path) -> list[tuple[str, ParsedAictx]]:
    ...
    results.sort(key=lambda x: (x[0] != ".", x[0].count("/"), x[0]))
    return results


def _walk(root: Path):
    """Yield .context.aictx files, root first, then children."""
