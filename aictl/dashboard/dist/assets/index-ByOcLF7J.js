...
`),P=O.length;if(P<=sn*3||l)return c`${x(O,1)}
      ...
      <div class="prev-actions">
        ...
        <button class="prev-btn" onClick=${()=>m.openViewer(e.path)}>open in viewer</button>
      </div>`},j=e.size>0?Math.round(e.size/60):0;return c`<div>
    <button class="fitem" onClick=${$} aria-expanded=${s} title=${e.path}>
      ...
      <span class="fmeta">
        ...
        ${ge(e.size)}${j?c` <span class="text-muted">${j}ln</span>`:""}${e.tokens?c` <span class="text-muted">${z(e.tokens)}t</span>`:""}
  ...
  </div>`}function Co({tool:e,root:t}){var w,j,A,O,P,b,C,D;const[s,n]=G(!1),{snap:l,history:o}=Xe(je),a=ie(()=>((l==null?void 0:l.tool_configs)||[]).find(F=>F.tool===e.tool),[l,e.tool]),i=ie(()=>{var F;return(F=o==null?void 0:o.by_tool)==null?void 0:F[e.tool]},[o,e.tool]),r=Oe[e.tool]||"var(--fg2)",d=$t[e.tool]||"🔹",v=e.files.reduce((F,R)=>F+R.tokens,0),p=e.processes.filter(F=>F.anomalies&&F.anomalies.length).length,m=zo(e.live),g=(((w=e.live)==null?void 0:w.outbound_rate_bps)||0)+(((j=e.live)==null?void 0:j.inbound_rate_bps)||0),k=e.processes.reduce((F,R)=>F+(parseFloat(R.cpu_pct)||0),0),M=e.processes.reduce((F,R)=>F+(parseFloat(R.mem_mb)||0),0),T=ie(()=>Math.max(...e.processes.map(F=>parseFloat(F.mem_mb)||0),100),[e.processes]),y=(((O=(A=e.token_breakdown)==null?void 0:A.telemetry)==null?void 0:O.errors)||[]).length,$=ie(()=>{const F={};return e.files.forEach(R=>{const q=R.kind||"other";(F[q]=F[q]||[]).push(R)}),Object.keys(F).sort((R,q)=>{const Y=ja.indexOf(R),ne=ja.indexOf(q);return(Y<0?99:Y)-(ne<0?99:ne)}).map(R=>({kind:R,files:F[R]}))},[e.files]),x="tcard"+(s?" open":"")+(p||y?" has-anomaly":"");return c`<div class=${x}>
    ...
    ${s&&c`<div class="tcard-body">
      ${((P=Na[e.tool])==null?void 0:P.length)>0&&c`<div class="tool-relationships">
        ${Na[e.tool].map(F=>c`<span key=${F.label} class="rel-badge rel-${F.type}"
  ...
  </div>`}function dp({mem:e}){var x;const[t,s]=G(!1),[n,l]=G(!1),[o,a]=G(null),[i,r]=G(null),[d,v]=G(!1),p=Xe(je),m=(e.file||"").replace(/\\/g,"/").split("/").pop(),g=qe(async()=>{if(t){s(!1);return}if(s(!0),ol.has(e.file)){a(ol.get(e.file));return}v(!0),r(null);try{const w=await Ro(e.file);a(w)}catch(w){r(w.message)}finally{v(!1)}},[t,e.file]),k=(w,j)=>w.map((A,O)=>c`<span class="pline"><span class="ln">${j+O}</span>${X(A)||" "}</span>`),M=()=>{if(d)return c`<span class="loading-state">Loading...</span>`;if(i)return c`<span class="error-state">${i}</span>`;if(!o)return null;const w=o.split(`
`),j=w.length;if(j<=sn*3||n)return c`${k(w,1)}
        <div class="prev-actions">
        ...
        </div>`;const A=w.slice(-sn),O=j-sn+1;return c`${k(A,O)}
      <div class="prev-actions">
  ...
  </div>`}function vp(){const{snap:e}=Xe(je);if(!e||!e.agent_memory.length)return c`<p class="empty-state">No agent memory found.</p>`;const t=ie(()=>{const s={};return e.agent_memory.forEach(n=>{(s[n.source]=s[n.source]||[]).push(n)}),Object.entries(s)},[e.agent_memory]);return c`<${fp}/>
    ${t.map(([s,n])=>c`<${pp} key=${s} source=${s} entries=${n}/>`)}`}function mp(){var n,l,o,a;const{snap:e}=Xe(je);if(!e)return c`<p class="empty-state">Loading...</p>`;const t=e.tools.filter(i=>i.live).sort((i,r)=>{var d,v,p,m;return(((d=r.live)==null?void 0:d.outbound_rate_bps)||0)+(((v=r.live)==null?void 0:v.inbound_rate_bps)||0)-((((p=i.live)==null?void 0:p.outbound_rate_bps)||0)+(((m=i.live)==null?void 0:m.inbound_rate_bps)||0))}),s=Object.entries(e.live_monitor&&e.live_monitor.diagnostics||{});return c`<div class="live-stack">
  ...
  </div>`}function hp(){var M,T,y,$;const{snap:e,globalRange:t}=Xe(je),[s,n]=G(null),[l,o]=G([]),[a,i]=G(null),r=ie(()=>e?e.tools.filter(x=>!x.meta&&(x.files.length||x.processes.length||x.live)).sort((x,w)=>x.label.localeCompare(w.label)):[],[e]);if(de(()=>{!s&&r.length&&n(r[0].tool)},[r,s]),de(()=>{if(!s||!t)return;let x="/api/events?tool="+encodeURIComponent(s)+"&since="+t.since+"&limit=500";t.until!=null&&(x+="&until="+t.until),fetch(x).then(w=>w.json()).then(o).catch(()=>o([]))},[s,t]),de(()=>{if(!s||!t)return;let x="/api/history?since="+t.since+"&tool="+encodeURIComponent(s);t.until!=null&&(x+="&until="+t.until),fetch(x).then(w=>w.json()).then(w=>{var j;return i(((j=w==null?void 0:w.by_tool)==null?void 0:j[s])||null)}).catch(()=>i(null))},[s,t]),!e)return c`<p class="loading-state">Loading...</p>`;const d=r.find(x=>x.tool===s),v=(M=e.tool_telemetry)==null?void 0:M.find(x=>x.tool===s),p=d==null?void 0:d.live,m=Oe[s]||"var(--fg2)",g={month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",hourCycle:"h23"},k=t.until!=null?new Date(t.since*1e3).toLocaleString(void 0,g)+" – "+new Date(t.until*1e3).toLocaleString(void 0,g):"";return c`<div class="es-layout">
      ...
      ${s&&c`<Fragment>
        ...
        <div class="es-section">
          ...
          ${l.length?c`<div class="es-feed">
            ${l.map((x,w)=>{const j=Yc[x.kind]||"var(--fg2)",A=new Date(x.ts*1e3).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit",hourCycle:"h23"}),O=x.detail?Object.entries(x.detail).map(([P,b])=>P+"="+b).join(", "):"";return c`<div key=${x.ts+"-"+x.tool+"-"+w} class="es-event">
      ...
      <div style="overflow-x:auto">
        <table role="table" aria-label="Tool telemetry" style="width:100%">
          ...
          ${M.length>1&&c`<tr class="text-bolder" style="border-top:2px solid var(--border)">
            ...
            <td style="text-align:right" class="text-muted">${z(j)}</td>
  ...
  </div>`}function Sp({session:e}){const{snap:t}=Xe(je),s=t&&t.agent_memory||[],n=e.project||"",l=n?s.filter(o=>{const a=o.project||o.tool||"";return!n||a.includes(n.replace(/\\/g,"/").split("/").pop())}):s;return l.length?c`<div class="mono text-xs" style="max-height:20rem;overflow-y:auto">
    ...
    <div class="stl-chart" style=${"height:"+w+"px"}>
      ...
      <div class="stl-axis" style=${"top:"+(w-g)+"px"}>
        ${j.map(O=>c`<span key=${O.ts} class="stl-tick"
  ...
  </div>`}function Fp(){const{snap:e}=Xe(je),t=e&&e.agent_teams||[];if(!t.length)return null;const s=t.reduce((o,a)=>o+a.agent_count,0),n=t.reduce((o,a)=>o+(a.total_input_tokens||0),0),l=t.reduce((o,a)=>o+(a.total_output_tokens||0),0);return c`<div class="es-section" style="margin-bottom:var(--sp-6)">
  ...
  </div>`}function jp(){const{snap:e,globalRange:t,rangeSeconds:s,enabledTools:n}=Xe(je),[l,o]=G([]),[a,i]=G(!1),[r,d]=G(!0),[v,p]=G(null),[m,g]=G(null),[k,M]=G([]);de(()=>{d(!0),i(!1),fetch("/api/sessions?active=false").then(b=>b.json()).then(b=>{o(b),d(!1)}).catch(()=>{i(!0),d(!1)})},[]),de(()=>{if(!t)return;let C="/api/session-timeline?since="+Math.min(t.since,Date.now()/1e3-86400);t.until!=null&&(C+="&until="+t.until),fetch(C).then(D=>D.json()).then(M).catch(()=>M([]))},[t]),de(()=>{const b=C=>{const D=C.detail;D&&D.session_id&&(p(D.session_id),g(D))};return window.addEventListener("aictl-session-select",b),()=>window.removeEventListener("aictl-session-select",b)},[]);const T=b=>n===null||n.includes(b),y=(e&&e.sessions||[]).filter(b=>T(b.tool)),$=l.filter(b=>T(b.tool)),x=k.filter(b=>T(b.tool));let w=y.find(b=>b.session_id===v);if(!w&&v){const C=l.find(D=>D.session_id===v)||m;C&&C.session_id===v&&(w={session_id:C.session_id,tool:C.tool,project:C.project||"",duration_s:C.duration_s||0,active:C.active||!1,started_at:C.started_at,ended_at:C.ended_at,files_touched:[],files_loaded:[],exact_input_tokens:C.input_tokens||0,exact_output_tokens:C.output_tokens||0,pids:[],file_events:C.files_modified||0})}const j=b=>{p(C=>C===b.session_id?null:b.session_id)},A={};for(const b of y){const C=b.project||"Unknown Project";A[C]||(A[C]=[]),A[C].push(b)}const O=Object.keys(A).sort();return c`<div>
    <div class="mb-lg">
      ...
      ${y.length?O.length>1?O.map(b=>c`<div key=${b} style="margin-bottom:var(--sp-5)">
              ...
              <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:var(--sp-5)">
                ${A[b].map(C=>c`<${Pi} key=${C.session_id} session=${C}
                  onSelect=${j} isSelected=${C.session_id===v} agentTeams=${e==null?void 0:e.agent_teams}/>`)}
              </div>
              ...
              ${y.map(b=>c`<${Pi} key=${b.session_id} session=${b}
                onSelect=${j} isSelected=${b.session_id===v}/>`)}
    ...
    <div class="sf-seq-container">
      ${g?c`<div class="loading-state" style="padding:var(--sp-8)">Loading session flow...</div>`:j.length===0?c`<div class="empty-state" style="padding:var(--sp-8)">
            ...
            <div class="sf-seq-body">
              ${j.map((P,b)=>P._from&&P._to?c`<${af} key=${b} event=${P} participants=${A}
  ...
  </div>`}const tl=new Set(["claude-code","claude-desktop","copilot","copilot-vscode","copilot-cli","copilot-jetbrains","copilot-vs","copilot365","codex-cli","gemini","gemini-cli"]);function Rf({snap:e,enabledTools:t,onToggle:s,onSetAll:n}){if(!e)return null;const l=e.tools.filter(a=>!a.meta);if(!l.length)return null;const o=t===null;return t?t.length:l.length,c`<div class="tool-filter-bar">
  ...
  `}function jf(){var ne;const[e,t]=nr(Pf,Of),{snap:s,history:n,connected:l,activeTab:o,globalRange:a,searchQuery:i,theme:r,viewerPath:d,events:v,enabledTools:p}=e,[m,g]=G(null),k=lt(null);de(()=>{document.documentElement.setAttribute("data-theme",r);try{localStorage.setItem("aictl-theme",r)}catch{}},[r]);const M=qe(()=>{t({type:"SET_THEME",payload:Gl[(Gl.indexOf(r)+1)%Gl.length]})},[r]),T=qe(L=>{const B=L.since,V=L.until!=null?"&until="+L.until:"";L.id==="live"?g(null):L.id!=="custom"?fetch("/api/history?range="+L.id).then(H=>H.json()).then(g).catch(()=>{}):fetch("/api/history?since="+B+V).then(H=>H.json()).then(g).catch(()=>{}),fetch("/api/events?since="+B+V).then(H=>H.json()).then(H=>t({type:"EVENTS_INIT",payload:H})).catch(()=>{})},[]);de(()=>{let L,B=1e3,V=!1,H=!1;fetch("/api/snapshot").then(U=>U.json()).then(U=>t({type:"SNAP_REPLACE",payload:U})).catch(()=>{}),fetch("/api/history").then(U=>U.json()).then(U=>t({type:"HISTORY_INIT",payload:U})).catch(()=>{}),T(a);function le(){V||(L=new EventSource("/api/stream"),L.onmessage=U=>{const Le=JSON.parse(U.data);t({type:"SSE_UPDATE",payload:Le}),B=1e3},L.onerror=()=>{t({type:"SET_CONNECTED",payload:!1}),L.close(),V||setTimeout(le,B),B=Math.min(B*2,3e4)})}le();const oe=setInterval(()=>{V||H||(H=!0,fetch("/api/snapshot").then(U=>U.json()).then(U=>t({type:"SNAP_REPLACE",payload:U})).catch(()=>{}).finally(()=>{H=!1}))},3e4);return()=>{V=!0,L&&L.close(),clearInterval(oe)}},[]);const y=qe(L=>{const B=cl[L]||3600,V={id:L,since:Date.now()/1e3-B,until:null};t({type:"SET_RANGE",payload:V}),Qs("range",L),T(V)},[T]),$=qe((L,B)=>{const V={id:"custom",since:L,until:B};t({type:"SET_RANGE",payload:V}),T(V)},[T]),x=a.id==="live"?n:m||n,w=a.until?a.until-a.since:cl[a.id]||3600;de(()=>{const L=B=>{var V;if(B.key==="Escape"&&t({type:"SET_VIEWER",payload:null}),B.key==="/"&&document.activeElement!==k.current&&(B.preventDefault(),(V=k.current)==null||V.focus()),document.activeElement!==k.current){const H=ro.find(le=>le.key===B.key);H&&(t({type:"SET_TAB",payload:H.id}),Qs("active_tab",H.id))}};return document.addEventListener("keydown",L),()=>document.removeEventListener("keydown",L)},[]);const j=qe(L=>t({type:"SET_VIEWER",payload:L}),[]),A=qe(L=>{if(!tl.has(L))return;const B=s?s.tools.filter(H=>H.tool!=="aictl"&&H.tool!=="any"&&tl.has(H.tool)).map(H=>H.tool):[];let V;p===null?V=B.filter(H=>H!==L):p.indexOf(L)>=0?V=p.filter(le=>le!==L):(V=[...p,L],V.length>=B.length&&(V=null)),t({type:"SET_TOOL_FILTER",payload:V}),Qs("tool_filter",V)},[s,p]),O=qe(L=>{t({type:"SET_TOOL_FILTER",payload:L}),Qs("tool_filter",L)},[]),P=ie(()=>{if(!s)return s;let L=s.tools;if(L=L.filter(B=>tl.has(B.tool)||B.tool==="aictl"),p!==null&&(L=L.filter(B=>p.includes(B.tool)||B.tool==="aictl")),i){const B=i.toLowerCase();L=L.filter(V=>V.label.toLowerCase().includes(B)||V.tool.toLowerCase().includes(B)||V.vendor&&V.vendor.toLowerCase().includes(B)||V.files.some(H=>H.path.toLowerCase().includes(B))||V.processes.some(H=>(H.name||"").toLowerCase().includes(B)||(H.cmdline||"").toLowerCase().includes(B))||V.live&&((V.live.workspaces||[]).some(H=>H.toLowerCase().includes(B))||(V.live.sources||[]).some(H=>H.toLowerCase().includes(B))))}return{...s,tools:L}},[s,i,p]),b=ie(()=>{var V;const L=Date.now()/1e3-300,B=new Map;for(const H of v)if(H.kind==="file_modified"&&H.ts>=L&&((V=H.detail)!=null&&V.path)){const le=B.get(H.detail.path);(!le||H.ts>le.ts)&&B.set(H.detail.path,{ts:H.ts,growth:H.detail.growth_bytes||0,tool:H.tool})}return B},[v]),C=ie(()=>({snap:P,history:n,openViewer:j,recentFiles:b,globalRange:a,rangeSeconds:w,enabledTools:p}),[P,n,j,b,a,w,p]),D={overview:()=>c`
    ...
    <${Mf}/>
  </${je.Provider}>`}Ic(c`<${jf}/>`,document.getElementById("app"));
