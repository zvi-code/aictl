...
class MonitorRuntime:
    ...
    def __init__(self, config: MonitorConfig) -> None:
        ...
        self.correlator = SessionCorrelator(config, workspace_sizes=self.workspace_sizes)
    ...
    async def run_live(self) -> int:
        ...
        consumer_task = asyncio.create_task(self._consume(), name="monitor-consumer")
        collector_tasks = [
            asyncio.create_task(collector.run(self.queue.put), name=f"collector:{collector.name}")
    ...
    def _build_collectors(self):
        collectors = [PsutilProcessCollector(self.config), _select_network_collector(self.config)]
...
def _select_network_collector(config: MonitorConfig):
    ...
    if system == "Darwin" and MacOSNetworkCollector.is_supported():
        return MacOSNetworkCollector(config.network_interval)
