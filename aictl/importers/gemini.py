...
def import_from(root: Path) -> ImportResult | None:
    ...
    root_base, profile_name, profile_text = read_root_scope(root / "GEMINI.md", root / "GEMINI.local.md", NAME)
    ...
    capabilities = read_cap_dir(gemini_dir / "commands", "*.md", "command", NAME) + read_skills_dir(
        gemini_dir / "skills", NAME
    )
